...
% Modified by Gail Rosenbaum, Yelina Chen, Haoxue Fan
...
Fit.LB = [0 1e-6 0];
Fit.UB = [3.5 10 1];
Fit.Nparms = size(Fit.UB,2); %alpha,beta,weight_frd
...
for s = 1:length(subs)
    ...
    for iter = 1:niter  
        ...
        [res,lik] = ...
            fmincon(@(x) revMod_model(subdata,x(1),x(2),x(3)),...
        ...
        Fit.Result.weight_frd(s, iter) = res(3);
...
fittable = array2table(fitmat,'VariableNames',{'Idx','Alpha','Beta','weight_frd','LogLik'});
